...
这个 skill 默认服务目标是“一轮交付可用版本”，而不是让不懂提示词的用户反复确认。Codex 内部会跑三道自检：

1. **内容充实闸门**：避免页面太空，每页要有观点、资料依据和有意义的图表结构。
2. **风格锁定闸门**：先锁定视觉系统，再生成整套页面，减少风格漂移。
3. **最终清理闸门**：删除无意义的 X、随机图标、假标签、未要求的 Q&A，以及不支撑汇报内容的装饰元素。

正式开始前，skill 会先执行一个有边界的 grill-me 式开工追问：先检查用户已经给的资料，把缺失需求标成开放决策分支，每轮只问 1-3 个必要问题，然后记录默认假设、开放风险和信心等级，再进入一轮生成。
...
在正式规划和生成前，先检查已有资料，关闭能直接判断的决策分支，再确认缺失的关键需求：主题标题、受众、时长或页数、参考 PPT、资料文件、必须出现的关键词和明确不要出现的内容。低风险细节使用默认值并说明假设。
...
默认直接生成整套页面图片；只有用户主动要求先确认少量页面，或资料/风格存在明显歧义时，才分批确认。图片会按页码保存，作为最后展示效果的视觉源文件。
...
- 是否有无意义的 X、随机图标、假标签或不支撑汇报内容的装饰元素

### 8. 把最终图片插入 PPTX

把通过 QA 的最终页面图片全屏插入 PPTX。这里的 PPTX 是承载文件，核心视觉仍然来自 image2 生成的页面图片。
...
- **有边界地先问清楚**：先检查资料并关闭缺失决策分支，再默认交付完整第一版。
- **一轮优先**：默认一次生成完整可用版本，只在用户要求或风险较高时分批确认。
