...
正式开始前，skill 会先执行一个 grill-me 式的开工追问：一次性补齐缺失的关键需求，包括主题标题、受众、时长或页数、资料来源、参考风格、必须出现的内容、明确不要出现的内容，以及最终要交付的文件。可默认的细节不反复追问，保证整体仍然尽量一轮完成。
...
在正式规划和生成前，先确认缺失的关键需求：主题标题、受众、时长或页数、参考 PPT、资料文件、必须出现的关键词和明确不要出现的内容。低风险细节使用默认值并说明假设。
...
- **开工先问清楚**：先一次性补齐缺失的关键需求，再默认交付完整第一版。
